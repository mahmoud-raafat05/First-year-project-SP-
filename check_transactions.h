...
void checkTransactions(User users[], int userIdx) {
    ...
    for (int i = 0; i < users[userIdx].transactionCount; i++) {
        ...
        if (users[userIdx].transactions[i].from == users[userIdx].username &&
            users[userIdx].transactions[i].to != users[userIdx].username) {
    ...
    if (!hasTransactions) {
        ...
        cout << "Total Sent: " << sentTotal << " | Total Received: " << receivedTotal << endl;
    }
}